package com.lifelink.repository;

import com.lifelink.model.BloodInventory;
import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.stereotype.Repository;
import java.util.List;

@Repository
public interface BloodInventoryRepository extends JpaRepository<BloodInventory, Integer> {
    List<BloodInventory> findByHospitalId(Integer hospitalId);
    List<BloodInventory> findByBloodGroupAndStatus(String bloodGroup, String status);
    List<BloodInventory> findByHospitalIdAndBloodGroupAndStatus(Integer hospitalId, String bloodGroup, String status);
}
